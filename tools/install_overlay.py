...
def patch_project(path: Path) -> None:
    ...
    text = replace_once(text, 'config/name="Godot Third-Person Shooter Demo"', 'config/name="Drylands Relay"', "project name")
...
def patch_robot(path: Path) -> None:
    text = path.read_text(encoding="utf-8")
    text = replace_once(text, 'const SHOOT_WAIT: float = 6.0', 'const SHOOT_WAIT: float = 2.8', "robot cadence")
    text = replace_once(text, 'const AIM_TIME: float = 1.0', 'const AIM_TIME: float = 0.65', "robot aim")
    text = replace_once(
        text,
...
def patch_level(path: Path) -> None:
    text = path.read_text(encoding="utf-8")
    text = replace_once(text, 'await get_tree().create_timer(15.0).timeout', 'await get_tree().create_timer(3.0).timeout', "respawn tempo")
    path.write_text(text, encoding="utf-8")
...
def install(base: Path, overlay: Path) -> None:
    ...
    if missing:
        raise RuntimeError("Not a compatible TPS demo checkout. Missing: " + ", ".join(missing))

    drylands_src = overlay / "drylands"
    drylands_dst = base / "drylands"
    if drylands_dst.exists():
        shutil.rmtree(drylands_dst)
    shutil.copytree(drylands_src, drylands_dst)
    shutil.copy2(overlay / "drylands/export_presets.cfg", base / "export_presets.cfg")
    ...
    patch_project(base / "project.godot")
    patch_player(base / "player/player.gd")
    patch_robot(base / "enemies/red_robot/red_robot.gd")
    patch_level(base / "level/level.gd")
...
def main() -> None:
    ...
    parser.add_argument("project", type=Path, help="Path to the pinned tps-demo checkout")
    parser.add_argument("--overlay", type=Path, default=Path(__file__).resolve().parents[1])
    args = parser.parse_args()
    install(args.project.resolve(), args.overlay.resolve())
    print(f"Drylands overlay installed into {args.project.resolve()}")
